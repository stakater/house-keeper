...
instances=$(instances)
while [[ count -lt 120 && ${#k[@]} != 0 ]]
do
  echo "Waiting for $instances[@]"
  sleep 5
  for instance in "${instances[@]}"; do
    state=`docker exec $CONTAINER_NAME aws ec2 describe-instances --region $region --query "Reservations[].Instances[?InstanceId=='i-07f4a6a10db3e4075'].State.Name" --output text`
